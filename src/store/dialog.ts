...
function createStore(instanceId: string = 'default') {
    const storeBaseModalInstance = storeBaseModal(instanceId);
    return create<DialogStore<any>>((set, get) => ({
        instances: [],
        ...
        addInstance: (instance: ConfirmInstance) => {
            ...
            instance.id = storeBaseModalInstance.getState().actions.pushModal(instance.id, React.createElement(ModalWindow, { config: instance.config, modalWindowId: instance.id, handleClose: get().handleClose, handleAction: get().handleAction }));
        },
        ...
        handleClose: (id: string, isForceCancel: boolean = false) => {
            ...
            if (!modal) return;
            storeBaseModalInstance.getState().actions.popModal(id);
...
export default function BaseDialogStore(instanceId: string = "default"): BaseDialogStoreInstance {
    if (!storeMap.has(instanceId)) storeMap.set(instanceId, createStore(instanceId));
    return storeMap.get(instanceId)! as BaseDialogStoreInstance;
}
